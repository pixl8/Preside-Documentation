...
The customization handler should return a string of the rendered viewlet and is supplied an `args` structure. At minimum this includes `objectName`. When you call the core listing (or `objectDataTable()`), the same `args` also carry listing toolbar flags such as `allowColumnPicker`, `allowColumnFilter`, `allowSavedViews`, `compact` and `listingPreferenceKey` — see [[customizingdatamanager]].
